	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if("github".equals(oAuth2AuthenticationToken.getAuthorizedClientRegistrationId()) || 
			...
			if(user == null) {
				...
				Authentication securityAuth =  createAuthentication(oAuth2User,
						authorities,
...
//			    authoritiesFromDefaultOAuth2User
//			);
		
		System.out.println("user details: " + userDetails);
		
		// Explicitly set this as the principal
		Authentication securityAuth = new UsernamePasswordAuthenticationToken(
		    userDetails, null, authoritiesFromDefaultOAuth2User
		);
	...
	//private method that create authentication object that contains OAuth2AuthenticationToken, list of authorities,DefaultOAuth2User
	private Authentication createAuthentication(DefaultOAuth2User oauthUser,
			List<SimpleGrantedAuthority> authorities, OAuth2AuthenticationToken oAuth2AuthenticationToken) {
		//create the authentication object
		return  new OAuth2AuthenticationToken(oauthUser,
				authorities,
	...
	private User createNewUser(String email,String username, OAuth2AuthenticationToken oAuth2AuthenticationToken) {
		...
		// Assign a dummy password to avoid database constraint issues
		//this is because OAuth2 authentication does not required a password. and the password field is annotated as
		//not null for users who register using email
